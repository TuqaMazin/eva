...
class ReminderController extends GetxController {
  ...
  onReminderTimeClick(BuildContext context, int index) async {
    ...
    if (notificationsEnabled && context.mounted) {
      await showTimePickerDialog(context, index: index);
      await DBHelper.dbHelper.updateReminderTime(reminderList[index].rid!, timeController.text).then((value) async {
  ...
  Future<bool> _requestPermissions() async {
    if (Platform.isIOS || Platform.isMacOS) {
      ...
      final bool? grantedAlarmPermission = await androidImplementation?.requestExactAlarmsPermission();

      return notificationsEnabled = (grantedNotificationPermission! && grantedAlarmPermission!);
    }
    return false;
  }
  ...
  onRepeatDaysClick(BuildContext context, int index) async {
    ...
    await DBHelper.dbHelper.updateReminderDays(reminderList[index].rid!, repeatController.text, repeatNo!).then((value) async {
      await _getDataFromDatabase();
    });
  ...
  onChangeReminderOnOrOff(bool value, ReminderTable reminderList) async {
    await DBHelper.dbHelper.updateReminderStatus(reminderList.rid!, ((value) ? Constant.valueOne : Constant.valueZero)).then((value) async {
      await _getDataFromDatabase();
    });
        ...
        child: Wrap(
          ...
          children: <Widget>[
            Column(
              ...
              children: [
                Container(
                  margin: EdgeInsets.symmetric(vertical: AppSizes.height_3, horizontal: AppSizes.width_6),
                  child: Text(
                    "txtTip".tr,
                ...
                Container(
                  margin: EdgeInsets.only(bottom: AppSizes.height_3, left: AppSizes.width_6, right: AppSizes.width_6),
                  child: Text(
                    "${"txtDeleteDes".tr}?",
                    textAlign: TextAlign.left,
                    style: TextStyle(
